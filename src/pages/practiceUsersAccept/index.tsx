...
import { useGetPracticeProfilesQuery, useUpdatePracticeProfileMutation } from '../../redux/slices/practiceProfileSlice';
import Swal from 'sweetalert2';
import UserDetailsModal from '../components/userDeatilsPracticeViewModal';
import { FaEye, FaFilter } from "react-icons/fa";

const UserAccept = () => {
  const [filter, setFilter] = useState<Status>('all');
  const { data: locumProfiles, isLoading, error } = useGetPracticeProfilesQuery();
  const [updateLocumProfile] = useUpdatePracticeProfileMutation();
  ...
  const approvedUsers = locumProfiles?.filter(
    (user: any) => user.status === 'accept' && user.email !== "admin@gmail.com"
  );
  ...
  type Status = 'all' | 'pending' | 'accept' | 'delete'|'verify';
  ...
  const getFilteredUsersByStatus = (status: Status) => {
    let users = [];
    if (status === 'all') {
      users = locumProfiles?.filter((user: any) => user.email !== "admin@gmail.com") || [];
    } else {
      users = locumProfiles?.filter((user: any) => user.status === status && user.email !== "admin@gmail.com") || [];
    }
    return users;
  };
  const filteredUsers = getFilteredUsersByStatus(filter);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-2xl font-bold text-gray-900">Practice User Management</h2>
